...
describe.concurrent("transformObjectId", () => {
	const schema = z.object({
		_id: transformObjectId,
	});
	it("should transform a valid ObjectId string to an ObjectId instance", () => {
		const result = schema.safeParse({ _id: "60ad0efd6c776f2e4e7b8b52" });
		expect(result.success).toBe(true);
		expect(result.data?._id).toBeInstanceOf(ObjectId);
	});

	it("should fail when the ObjectId string is invalid", () => {
		const result = schema.safeParse({ _id: "invalid-object-id" });
...
describe.concurrent("transformDate", () => {
	...
	it("should fail when the date string is invalid", () => {
		...
		expect(result.success).toBe(false);
		expect(result.error?.issues[0].code).toBe(z.ZodIssueCode.invalid_date);
	});
});
